...
static int system_board(struct ubus_context *ctx, struct ubus_object *obj,
  ...
  if ((f = fopen("/proc/cpuinfo", "r")) != NULL) {
    while (fgets(line, sizeof(line), f)) {
      ...
      if (!strcasecmp(key, "system type") || !strcasecmp(key, "processor") ||
          !strcasecmp(key, "cpu") || !strcasecmp(key, "model name")) {
        strtoul(val + 2, &key, 0);
  ...
  if ((f = fopen("/tmp/sysinfo/model", "r")) != NULL ||
    ...
    while (fgets(line, sizeof(line), f)) {
      ...
      if (!strcasecmp(key, "machine") || !strcasecmp(key, "hardware")) {
...
static int sysupgrade(struct ubus_context *ctx, struct ubus_object *obj,
  ...
  /* sysupgrade_exec_upgraded() will never return unless something has gone
   * wrong */
  return UBUS_STATUS_UNKNOWN_ERROR;
}
